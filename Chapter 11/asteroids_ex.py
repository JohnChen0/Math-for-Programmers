...
BLACK_HOLE_COUNT = 3
# Mass of a black hole, for calculating strength of gravity.
BLACK_HOLE_MASS = 50

# The minimum distance to use while applying inverse-square law to calculate gravity.
MIN_DISTANCE_FORCE = 0.1
# The minimum distance to calculate direction of gravity.
# Below the threshold, a random direction is used.
MIN_DISTANCE_DIRECTION = 1e-5

# When True, use the average of speed at the begining and end of time period to
# calculate distance moved. When False, use the algorithm from the book (i.e.,
# use speed at the end of time period).
IMPROVED_MOTION_CALCULATION = True

# Whenever an object bounces from the wall, its speed is multiplied by this factor.
BOUNCE_DAMPEN_FACTOR = 0.8
...
#     self.rotation_angle += self.angular_velocity * milliseconds / 1000.0
    ...
    def move(self, milliseconds, thrust_vector=(0,0), gravity_sources=[]):
        ...
        # Speed to use for calculating distance moved
        if IMPROVED_MOTION_CALCULATION:
            vx, vy = (vx0 + self.vx) / 2, (vy0 + self.vy) / 2
        else:
            vx, vy = self.vx, self.vy
        dx, dy = vx * milliseconds / 1000.0, vy * milliseconds / 1000.0
        self.x, self.y = vectors.add((self.x,self.y), (dx,dy))

        if bounce:
            if (self.x < -10 and self.vx < 0) or (self.x > 10 and self.vx > 0):
                self.vx = - BOUNCE_DAMPEN_FACTOR * self.vx
            if (self.y < -10 and self.vy < 0) or (self.y > 10 and self.vy > 0):
                self.vy = - BOUNCE_DAMPEN_FACTOR * self.vy
        else:
            if self.x < -10:
                self.x += 20
            if self.y < -10:
...
for _ in range(BLACK_HOLE_COUNT):
    ...
    if BLACK_HOLE_COUNT > 1:
        black_hole.x = uniform(-5, 5)
        black_hole.y = uniform(-5, 5)
...
def gravitational_field(sources, x, y):
    ...
    for source in sources:
        ...
        if distance >= MIN_DISTANCE_DIRECTION:
            direction = ((x - source.x) / distance, (y - source.y) / distance)
        else:
            angle = uniform(0, 2 * pi)
            direction = (cos(angle), sin(angle))
        force = - source.gravity / max(distance, MIN_DISTANCE_FORCE) ** 2
        fields.append(vectors.scale(force, direction))
